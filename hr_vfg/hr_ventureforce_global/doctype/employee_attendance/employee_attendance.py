    ...
    def validate(self):   
        ...
        if hr_settings.period_from != 1:
            if month == 1:
                ...
                temp_month = month - 1
                start_year = year
            first_day = dt(start_year, temp_month, int(hr_settings.period_from))
        ...
        dor = frappe.db.get_value("Employee",self.employee,"relieving_date")
        for data in self.table1:
            data.public_holiday = 0
            data.weekly_off = 0
            if getdate(data.date) < doj:
                data.absent = 1
                self.total_absents +=1
                continue
            if dor and getdate(data.date) > dor:
                data.absent = 1
                self.total_absents +=1
                continue
                            ...
                            if holiday_flag:
                                ...
                                if total_time:
                                    ...
                                    if total_time >= timedelta(hours=hr_settings.double_overtime_after,minutes=0,seconds=0):
                                        data.late_sitting = total_time + total_time
                                        total_holiday_hours += total_time
                                    if (data.late_sitting or total_time) > timedelta(hours=hr_settings.threshold_for_additional_hours,minutes=0,seconds=0):
                                            data.additional_hours  =  (data.late_sitting or total_time) - timedelta(hours=hr_settings.threshold_for_additional_hours,minutes=0,seconds=0)
