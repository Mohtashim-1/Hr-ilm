		...
		for data in self.table_4:
			...
			if len(att) > 0:
				if self.type == "Short Leave":
					if att[0]["late"] == 1 and hr_settings.short_leave_apply_on_late == 0:
						frappe.throw("You cannot apply for short leave on late. Please enable settings.")
					if att[0]["half_day"] == 1 and hr_settings.short_leave_apply_on_halfday == 0:
						frappe.throw("You cannot apply for short leave on half day. Please enable settings.")
			...
			if self.type == "Short Leave":
				existing_shl = frappe.db.sql(""" select count(*) as num
					   ...
					   ON c.parent = p.name where p.type=%s and p.month=%s and c.employee_id=%s and c.name!=%s and p.docstatus=1""",(self.type,self.month,data.employee_id,data.name),as_dict=1)
				exst_num = 0
				if len(existing_shl) > 0:
					exst_num = float(existing_shl[0]['num'] or 0)
				
				#if (exst_num + 1) > float(hr_settings.short_leave_no_allowed):
				#	frappe.throw("Maximum limit for short leaves exceeded.")

				#if (float(data.no_of_hours) + exst_hrs) > float(hr_settings.short_leave_hours_allowed):
				#	frappe.throw("Adjustment Limit Exceded. Availed hours = {0} and new hours request = {1}".format(str(exst_hrs),str(data.no_of_hours)))
			#elif (float(data.no_of_hours) + exst_hrs) > 6:
				#frappe.throw("Adjustment Limit Exceded. Availed hours = {0} and new hours request = {1}".format(str(exst_hrs),str(data.no_of_hours)))
...
def adj_settle():
	docs = frappe.get_all("Attendance Adjustment",filters={"month":"March","workflow_state": "Approved by HR Manager"},fields=["name"])
	for rec in docs:
