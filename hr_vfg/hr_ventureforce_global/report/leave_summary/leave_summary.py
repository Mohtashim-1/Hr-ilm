...
def get_data(filters: Filters) -> List:
	...
	active_employees = frappe.get_list(
		"Employee",
		filters=conditions,
		fields=["name", "employee_name", "designation","date_of_joining","employee_code_old","department", "user_id", "leave_approver"],
	)
	...
	for employee in active_employees:
			...
			row.employee_code_old = employee.employee_code_old
			row.date_of_joining = employee.date_of_joining
			row.department = employee.department
			row.designation = employee.designation
			row.employee_name = employee.employee_name
			total_leave_balance = (row.get('closing_balance_casual') or 0) + \
							(row.get('closing_balance_annual') or 0) + \
							(row.get('closing_balance_medical') or 0)
			row.total_leave_balance = total_leave_balance
			...
			for leave_type in leave_types:
				...
				closing_balance_casual = row.closing_balance_casual or 0
				closing_balance_annual=row.closing_balance_annual or 0
				closing_balance_medical=row.closing_balance_medical or 0
				row.total_leave_balance = closing_balance_casual+closing_balance_annual+closing_balance_medical
