...
def get_data(filters):
		...
		if filters.department:
			dep="department='"+filters.department + "' and "
		emp = ""
		if filters.employee:
			emp = "employee='"+filters.employee + "' and "
		result = frappe.db.sql(""" select name from `tabEmployee Attendance` where {0}{1} month=%s and year=%s""".format(emp,dep),
			...
			for res in result:
				#try:
					...
					medical_l = len(frappe.db.get_all("Attendance", 
								{"attendance_date":["between",[start_date,end_date]],"status":"On Leave","docstatus":1,
									"employee":doc.employee,"leave_type":"Medical Leave"})) + (len(frappe.db.get_all("Attendance", 
								{"attendance_date":["between",[start_date,end_date]],"status":"Half Day","docstatus":1,
									"employee":doc.employee,"leave_type":"Medical Leave"}))/2)
					...
					deduction_days  =(float(doc.total_absents or 0) + float(doc.total_half_days/2 or 0) \
					                  + float(doc.lates_for_absent or 0)) - (annual_l + casual_l + medical_l)
					row={
							"emp_id":doc.employee,
							"biometric_id":doc.biometric_id,
							"name":doc.employee_name,
							"employee_code_old":frappe.db.get_value("Employee",{"name":doc.employee},"employee_code_old"),
							"department":doc.department,
							"employment_type":doc.employment_type,
							"date_of_joining":frappe.db.get_value("Employee",{"name":doc.employee},"date_of_joining"),
							"present_days":doc.present_days,
							"no_of_sundays":doc.no_of_sundays,
							"on_time":float(doc.present_days) - float(doc.total_lates),
							"total_lates":doc.total_lates,
							"half_days":doc.total_half_days/2,
							...
							"total_early_goings":doc.total_early_goings,
