...
def get_data(filters):
	...
	for item in records:
		...
		if item[12] == 1:
			status = "<span style='color:green;'>LT</span>" 
			total_lates+=1
		elif item[13] == 1:
			leave = frappe.db.get_value("Attendance", {"attendance_date":filters.get("to")},"leave_type")
			if leave:
				status = ""
				string_arr = leave.split(" ")
				for st in string_arr:
					status+=st[0]
